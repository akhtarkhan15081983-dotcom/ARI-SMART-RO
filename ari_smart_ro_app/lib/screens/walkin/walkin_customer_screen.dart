...
class WalkInCustomerScreen extends StatefulWidget {
  ...
  @override
  State<WalkInCustomerScreen> createState() => _WalkInCustomerScreenState();
}

class _WalkInCustomerScreenState extends State<WalkInCustomerScreen> {
  ...
  Future<void> _selectMachine() async {
    final AssetModel? asset = await Navigator.push<AssetModel>(
      context,
      MaterialPageRoute(builder: (_) => const AssetSelectionScreen()),
    );
  ...
  Future<void> _saveCustomer() async {
    ...
    try {
      final result = await customerService.createWalkInCustomer(
      ...
      if (result["success"] == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Unable to save customer")),
        );
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Walk-In Customer"), centerTitle: true),
      body: Form(
        key: formKey,
        child: ListView(
          padding: const EdgeInsets.all(16),
          children: [
            _textField(controller: nameController, label: "Customer Name"),
            ...
            _textField(controller: areaController, label: "Area"),

            _textField(controller: cityController, label: "City"),

            _textField(controller: stateController, label: "State"),
