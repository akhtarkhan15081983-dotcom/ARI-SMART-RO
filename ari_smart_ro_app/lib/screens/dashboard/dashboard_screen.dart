...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _startLiveLocationIfRequired() async { try { final role = await ApiService.getRole(); if (_normaliseRole(role) == 'ENGINEER') _liveLocationService.startTracking(); } catch (e) { debugPrint('LIVE LOCATION START ERROR: $e'); } }
  ...
  Future<void> _loadAttendance() async { try { final attendance = await _attendanceService.todayAttendance(); if (!mounted) return; setState(() { _todayAttendance = attendance; _isLoadingAttendance = false; }); } catch (e) { debugPrint('ATTENDANCE ERROR: $e'); if (mounted) setState(() => _isLoadingAttendance = false); } }
  Future<void> _loadRole() async { try { final storedRole = await ApiService.getRole(); if (!mounted) return; setState(() { _role = _normaliseRole(storedRole); _isLoadingRole = false; }); } catch (e) { if (!mounted) return; setState(() { _role = 'CUSTOMER'; _isLoadingRole = false; }); } }
  String _normaliseRole(String? role) { final value = role?.trim().toUpperCase().replaceAll('ROLE_', ''); switch (value) { case 'ADMIN': case 'MANAGER': case 'OFFICE': case 'ENGINEER': case 'CUSTOMER': return value!; default: return 'CUSTOMER'; } }
  ...
  Future<void> _logout() async { final confirm = await showDialog<bool>(context: context, builder: (_) => AlertDialog(title: const Text('Logout'), content: const Text('Are you sure you want to logout?'), actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')), ElevatedButton(onPressed: () => Navigator.pop(context, true), child: const Text('Logout'))])); if (confirm != true) return; await ApiService.logout(); if (!mounted) return; Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_) => const LoginScreen()), (route) => false); }
  ...
  void _handleItemTap(DashboardItem item) { switch (item.route) {
    ...
    case 'qr': _push(const QRScanScreen(jobId: 0)); return; case 'map': case 'engineer_map': _push(const EngineerMapScreen()); return;
    case 'walkin': _push(const WalkInCustomerScreen()); return; case 'service': _push(const ServiceListScreen()); return;
    case 'complaint': _push(const ComplaintListScreen()); return; case 'customers': _push(const CustomerListScreen()); return;
    case 'my_ro': _push(const MyROScreen()); return; case 'rent': _push(const RentPaymentScreen()); return;
    case 'rent_management': _push(const RentManagementScreen()); return; case 'payment_history': _push(const PaymentHistoryScreen()); return;
    case 'shop': _push(const ShopScreen()); return; case 'referral': _push(const ReferralScreen()); return;
    case 'history': _push(const CustomerHistoryScreen()); return; case 'profile': _push(const ProfileScreen()); return;
  ...
  @override Widget build(BuildContext context) {
    ...
    final items = _dashboardItems; final isCustomer = _role == 'CUSTOMER';
    return PopScope(canPop: false, onPopInvokedWithResult: (didPop, result) { if (!didPop) _handleDashboardBack(); }, child: Scaffold(
      ...
      body: RefreshIndicator(onRefresh: _loadDashboard, child: ListView(physics: const AlwaysScrollableScrollPhysics(), padding: const EdgeInsets.all(16), children: [
        if (!isCustomer) ...[Card(child: Padding(padding: const EdgeInsets.all(16), child: _isLoadingAttendance ? const Center(child: CircularProgressIndicator()) : Text(_todayAttendance == null ? 'Attendance not marked today' : 'Today attendance loaded'))), const SizedBox(height: 16)],
