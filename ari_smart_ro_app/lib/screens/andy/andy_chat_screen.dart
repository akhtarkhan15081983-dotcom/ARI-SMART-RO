...
enum _AndyAvatarState { idle, listening, thinking, confirming, speaking, completed }
...
class _AndyChatScreenState extends State<AndyChatScreen> {
  ...
  Future<String?> _sendText(String text, {bool autoSpeak = false}) async {
    ...
    try {
      ...
      if (!mounted) return null;
      setState(() {
        _messages.add(_ChatItem(false, 'ANDY local model se connect nahi ho pa raha.\n$e'));
    ...
    if (autoSpeak && answer != null && answer.isNotEmpty && _pendingActionId == null) {
      await _speak(answer);
    }
    return answer;
  }
  ...
  Future<void> _resolvePendingAction(bool confirm) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('ANDY action failed: $e')));
    } finally {
  ...
  Future<void> _speak(String text) async {
    if (_speaking) {
      ...
      if (mounted) setState(() {
        _speaking = false;
        _avatarState = _pendingActionId != null ? _AndyAvatarState.confirming : _AndyAvatarState.completed;
      });
      return;
    }
    try {
  ...
  String get _stateLabel {
    switch (_avatarState) {
      case _AndyAvatarState.listening: return 'LISTENING';
      case _AndyAvatarState.thinking: return 'THINKING';
      case _AndyAvatarState.confirming: return 'CONFIRMING';
      case _AndyAvatarState.speaking: return 'SPEAKING';
      case _AndyAvatarState.completed: return 'READY';
      case _AndyAvatarState.idle: return 'ONLINE';
    }
  }

  IconData get _stateIcon {
    switch (_avatarState) {
      case _AndyAvatarState.listening: return Icons.hearing;
      case _AndyAvatarState.thinking: return Icons.psychology_alt_outlined;
      case _AndyAvatarState.confirming: return Icons.verified_user_outlined;
      case _AndyAvatarState.speaking: return Icons.graphic_eq;
      case _AndyAvatarState.completed: return Icons.check_circle_outline;
      case _AndyAvatarState.idle: return Icons.smart_toy_outlined;
    }
  }
      ...
      child: Row(children: [
        AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: scheme.primary, width: _avatarState == _AndyAvatarState.speaking || _avatarState == _AndyAvatarState.listening ? 3 : 1),
          ),
          child: Icon(_stateIcon, size: 34, color: scheme.onPrimaryContainer),
        ),
        const SizedBox(width: 14),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('ANDY', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800)),
          ...
          Row(children: [
            ...
            Text(_stateLabel, style: TextStyle(fontWeight: FontWeight.w700, color: scheme.primary)),
          ]),
        ])),
      ]),
    );
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const Expanded(child: Text('Action confirmation required', style: TextStyle(fontWeight: FontWeight.w800))),
        ]),
        const SizedBox(height: 8),
        Text(_pendingActionSummary?.replaceAll('_', ' ') ?? 'Please confirm this ANDY action.'),
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: OutlinedButton.icon(
            onPressed: _resolvingAction ? null : () => _resolvePendingAction(false),
            icon: const Icon(Icons.close),
            label: const Text('Cancel'),
          )),
          const SizedBox(width: 10),
          Expanded(child: FilledButton.icon(
            onPressed: _resolvingAction ? null : () => _resolvePendingAction(true),
