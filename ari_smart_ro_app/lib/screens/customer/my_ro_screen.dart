...
class _MyROScreenState extends State<MyROScreen> {
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          ...
                          SizedBox(height: 10),
                          Text(
                            'No physical RO asset is assigned yet.',
                            textAlign: TextAlign.center,
                          ),
                        ],
                      ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          if (card.isNotEmpty)
            Text('Card $card', style: const TextStyle(color: Color(0xFFD5E6F0))),
          const SizedBox(height: 8),
          Text(
            '${ros.length} physical RO ${ros.length == 1 ? 'unit' : 'units'} linked',
            ...
            return Card(
              child: ListTile(
                leading: CircleAvatar(
                  child: Icon(
                    verified ? Icons.verified_user_rounded : Icons.engineering_outlined,
                  ),
                ),
                title: Text(
                  (engineer['name'] ?? 'ARI Engineer').toString(),
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'Service OTP • Job ${data['job_number'] ?? ''}',
  ...
  Widget _roPassport(Map<String, dynamic> ro) {
    ...
    final summary = Map<String, dynamic>.from(
      ro['component_summary'] as Map? ?? const {},
    );
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        (ro['ro_model_name'] ?? 'ARI RO').toString(),
                        style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900),
                      ),
                      Text('${ro['asset_id'] ?? ''} • Serial ${ro['serial_number'] ?? ''}'),
                      Text(
                        '${ro['deployment_type'] ?? ''} • ${ro['status'] ?? ''}',
            ...
            Wrap(
              ...
              children: [
                _chip(
                  'Active ${summary['active_parts'] ?? summary['active_records'] ?? active.length}',
            ...
            if (history.isNotEmpty) ...[
              ...
              ExpansionTile(
                ...
                title: Text(
                  'Replacement / removal history (${history.length})',
                  style: const TextStyle(fontWeight: FontWeight.w900),
                ),
  ...
  Widget _componentTile(Map<String, dynamic> part) {
    ...
    final lines = <String>[
      ...
      serialized
          ? (serial.isEmpty ? 'Serial pending' : 'Serial $serial')
          : 'Qty $quantity • no scan required',
