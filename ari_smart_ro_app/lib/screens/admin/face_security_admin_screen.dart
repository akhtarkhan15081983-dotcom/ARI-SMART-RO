...
class FaceSecurityAdminScreen extends StatefulWidget {
  ...
  @override
  State<FaceSecurityAdminScreen> createState() => _FaceSecurityAdminScreenState();
}
...
class _FaceSecurityAdminScreenState extends State<FaceSecurityAdminScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _engineers = engineers; _loading = false; });
    } catch (_) {
      if (!mounted) return;
      setState(() { _loading = false; _error = 'Unable to load engineers.'; });
    }
  }

  Future<void> _changePermission(Map<String, dynamic> engineer, bool allow) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(onPressed: () => Navigator.pop(context, true), child: Text(allow ? 'Allow' : 'Cancel Permission')),
        ],
      ),
    );
    ...
    setState(() => _busyId = id);
    try {
      final message = await _service.setReEnrollment(employeeId: id, allow: allow);
      if (!mounted) return;
                      ...
                      return Card(
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(e['name']?.toString() ?? 'Engineer', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
                              const SizedBox(height: 4),
                              Text('${e['employee_id'] ?? ''}  •  ${e['phone'] ?? ''}'),
                              const Divider(height: 24),
                              Text('Face: ${!enrolled ? 'Not enrolled' : verified ? 'Verified' : 'Enrolled - verification pending'}'),
                              const SizedBox(height: 5),
                              Text('Attendance device: ${deviceBound ? 'Bound' : 'Not bound'}'),
                              const SizedBox(height: 5),
                              Text('Re-enrollment: ${allowed ? 'ADMIN ALLOWED' : 'LOCKED'}', style: TextStyle(fontWeight: FontWeight.w700, color: allowed ? Colors.orange.shade800 : Colors.green.shade700)),
                              const SizedBox(height: 14),
                              SizedBox(
                                width: double.infinity,
                                child: allowed
                                    ? OutlinedButton.icon(
                                        onPressed: _busyId == id ? null : () => _changePermission(e, false),
                                        icon: const Icon(Icons.lock_outline),
                                        label: Text(_busyId == id ? 'Please wait...' : 'Cancel Re-enrollment Permission'),
                                      )
                                    : FilledButton.icon(
                                        onPressed: _busyId == id ? null : () => _changePermission(e, true),
                                        icon: const Icon(Icons.admin_panel_settings_outlined),
                                        label: Text(_busyId == id ? 'Please wait...' : 'Allow One Re-enrollment'),
                                      ),
                              ),
                            ],
                          ),
