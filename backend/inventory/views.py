    ...
    def create(self, request, *args, **kwargs):
        ...
        if inventory_item.status != "IN_STOCK":
            InventoryAuditLog.objects.create(
                inventory_item=inventory_item, performed_by=request.user,
                action="SECURITY_REJECT", old_status=inventory_item.status,
                new_status=inventory_item.status, serial_number=inventory_item.serial_number or "",
                remarks="Attempted to issue an inventory item that was not in stock.",
            )
        ...
        if EngineerBagItem.objects.filter(inventory_item=inventory_item, status="ISSUED").exists():
            InventoryAuditLog.objects.create(
                inventory_item=inventory_item, engineer=engineer, performed_by=request.user,
                action="SECURITY_REJECT", old_status=inventory_item.status,
                new_status=inventory_item.status, serial_number=inventory_item.serial_number or "",
                remarks="Duplicate issue attempt detected.",
            )
        ...
        InventoryAuditLog.objects.create(
            inventory_item=inventory_item, engineer=engineer, performed_by=request.user,
            action="ISSUED", old_status=old_status, new_status="ISSUED",
            serial_number=inventory_item.serial_number or "", remarks=request.data.get("remarks", ""),
        )
...
class OCRVerifyAPIView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        bag_item = EngineerBagItem.objects.select_related("inventory_item", "inventory_item__part", "engineer__user").filter(
            engineer=engineer, inventory_item__serial_number=serial_number,
            inventory_item__status="ISSUED", status="ISSUED",
        ).first()
        if not bag_item:
        ...
        return Response({
            "verified": True, "message": "Part verified successfully.",
            "inventory_item": bag_item.inventory_item.id,
            "part": bag_item.inventory_item.part.name,
            "part_code": bag_item.inventory_item.part.code,
            "serial_number": bag_item.inventory_item.serial_number,
        })
...
class MyBagAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return EngineerBagItem.objects.select_related("inventory_item__part", "engineer__user").filter(
            engineer__user=self.request.user, status="ISSUED", inventory_item__status="ISSUED",
        ).order_by("-issue_date")
...
class MyPartRequestsAPIView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        engineer = get_object_or_404(
            self.request.user.__class__.objects.none() if False else __import__('employees.models', fromlist=['EmployeeProfile']).EmployeeProfile.objects,
