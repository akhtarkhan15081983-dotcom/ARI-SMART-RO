...
class ROAsset(models.Model):
    ...
    current_customer = models.ForeignKey(
        Customer, on_delete=models.SET_NULL, null=True, blank=True, related_name="ro_assets"
    )
    ...
    reserved_for_customer = models.ForeignKey(
        Customer,
    ...
    def reservation_active(self):
        if self.reserved_for_customer_id is None:
            return False
        return self.reservation_expires_at is None or self.reservation_expires_at > timezone.now()
    ...
    def bom_verified(self):
        ...
        if not active.exists():
            return False
        return not active.filter(scan_status="PENDING").exists()

    @property
    def release_ready(self):
        return (
            self.is_active
            and self.status == "WAREHOUSE"
            and self.qc_status == "PASSED"
            and self.bom_verified
            and not self.reservation_active
        )
...
class ROAssetMovement(models.Model):
    ...
    customer = models.ForeignKey(
        Customer, on_delete=models.SET_NULL, null=True, blank=True, related_name="ro_asset_movements"
    )
...
class ROAssetComponent(models.Model):
    ...
    def warranty_end_date(self):
        ...
        return (self.installed_at + relativedelta(months=months)).date()
    ...
    def replacement_due_date(self):
        ...
        if days <= 0:
            return None
        return (self.installed_at + timezone.timedelta(days=days)).date()
...
class ROStockAuditItem(models.Model):
    ...
    scanned_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="scanned_ro_stock_audit_items",
    )
